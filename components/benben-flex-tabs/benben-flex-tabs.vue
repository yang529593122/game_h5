<template>
  <view
    :class="className"
    :style="{ 'box-sizing': 'content-box', 'padding-top': openTitleType ? StatusBarRpx + 'rpx' : '0rpx' }"
    :id="`${selectMark}-box`"
  >
  ...
  methods: {
    ...
    initObserveContent() {
      ...
      this.uGetRect(`#${this.selectMark}-box`).then((res) => {
        console.log('initObserveContent', res)
    ...
    pageScrollTo(name) {
      ...
      query.exec(function (res) {
        var miss = res[0].scrollTop + res[1].top - that.tabsHeight
        that.setflag()
        uni.pageScrollTo({
          scrollTop: miss,
          duration: 300,
