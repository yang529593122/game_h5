<template>
  <view class="fu-calendar-wrap">
    ...
    <view class="fu-calendar-data fu-flex fu-justify-center fu-align-center fu-margin fu-height-72" v-if="changeTitle">
      ...
      <view class="fu-title fu-text-32 fu-text-333">{{ currentDate[0] }}{{i18n['年']}}{{ currentDate[1] }}{{i18n['月']}}</view>
      <block v-if="changeMonth">
...
export default {
  props: {
    ...
    'start-text': {
      type: String,
      default: global.i18n['开始']
    },
    // 结束日期底部的提示文字
    'end-text': {
      type: String,
      default: global.i18n['结束']
    },
    // 选择日期开始结束当天背景色
    'active-bg-color': {
      type: String,
  ...
  data() {
    return {
      weekData: [global.i18n['日'], global.i18n['一'], global.i18n['二'], global.i18n['三'], global.i18n['四'], global.i18n['五'], global.i18n['六']], //星期
      calendar: [], //日历数组
      currentDate: [], //当前日期
      currentDay: '', //当天时间
      start: '', //开始时间
  ...
  methods: {
    ...
    getCalendar(y, m) {
      ...
      //这里塞入正常这个月的日期
      let week = [global.i18n['星期日'], global.i18n['星期一'], global.i18n['星期二'], global.i18n['星期三'], global.i18n['星期四'], global.i18n['星期五'], global.i18n['星期六']];
