...
export default {
  ...
  computed: {
    ...
    defaultValue() {
      let defaultValue = []
      if (this.mode == 'region' && this.value) {
        let code = this.value + ''
        defaultValue = [code.substring(0, 2) + '0000', code.substring(0, 4) + '00', code]
      }
      if ((this.mode == 'linkage' || this.mode == 'range') && this.value) {
        //判断this.value是否是数组
    ...
    pickerConfirm() {
      ...
      switch (this.mode) {
        ...
        case 'time':
          ...
          break
        case 'range':
          this.$emit('update:value', value.replace('至', ','))
          this.$emit('update:label', result)
          break
        case 'region':
          this.$emit('update:value', value[value.length - 1])
          this.$emit('update:label', result)
          break
        case 'selector':
