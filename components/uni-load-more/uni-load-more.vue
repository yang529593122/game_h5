<template>
	<view class="uni-load-more">
      ...
      <block v-if="isCart">
        <text class="line"></text>{{ status === 'more' ? i18n['上拉显示更多'] : status === 'loading' ? i18n['正在加载...'] : i18n['—— 我也是有底线的 ——'] }} <text class="line"></text>
      </block>
		<block v-else>
      {{ status === 'more' ? i18n['上拉显示更多'] : status === 'loading' ? i18n['正在加载...'] : i18n['没有更多数据了'] }}
    </block>
		</text>
	</view>
</template>
	...
	export default {
		...
		props: {
			...
			contentText: {
				...
				default () {
					return {
						contentdown: global.i18n['上拉显示更多'],
						contentrefresh:  global.i18n['正在加载...'],
						contentnomore:  global.i18n['没有更多数据了']
					}
				}
			},
      isCart:{
