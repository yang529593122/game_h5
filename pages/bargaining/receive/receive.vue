<template>
  <view class="applyFor">
    <view class="header">
      <fu-tab :tabs="BARGAINING_RECEIVE_NAV_DATAS" :current="current" node-title="title" node-key="status" current-key="id" :scrollspy="true"
        ...
        <view class="list-content" v-else>
            <view class="list-item" v-for="item in list">
                ...
                <view class="item-status">
                  <view class="item-num">
                    ...
                    <view class="item-num-right">
                      <!-- 出价记录状态：1=待付尾款,2=已拒绝,3=议价失败,4=已付尾款,5=超时未支付 -->
                      <text v-if="item.log_status === 0">议价中</text>
                      <text v-if="item.log_status === 1">买家待支付</text>
                      <text v-if="item.log_status === 2">议价失败</text>
                      <text v-if="item.log_status === 3">议价失败</text>
                      <text v-if="item.log_status === 4">交易成功</text>
                      <text v-if="item.log_status === 5">超时未支付</text>
                      <text v-if="item.log_status === 6">议价成功</text>
                    </view>
                  </view>
              ...
              <view class="item-content">
                ...
                <view class="item-info">
                  <view class="item-goods_name">
                   [{{ item.is_bao===1 ? '包赔':'押正包安' }}]{{ item.goods_name }}
                  </view>
              ...
              <view class="list-item-bottom">
                ...
                <view class="list-item-bottom-text" style="justify-content: flex-start;" v-if="item.log_status === 1">
                 <view class="status6" >
                   支付倒计时: {{ toHHmmss(item.pay_end_time*1000) }} 超时未支付将扣除定金
                 </view>
                </view>
                <view class="list-item-bottom-text">
                  ...
                  <view class="status0" v-if="item.log_status === 6 || item.log_status === 1 ">
                    议价定金¥{{ item.dicker_price }}
                    起拍金额¥{{ item.start_price }}
                    最高出价<text> ¥{{ item.max_price }}</text>
                  </view>
                ...
                <view class="list-item-btns">
                  ...
                  <view v-if="item.log_status===2 ||  item.log_status===3 || item.log_status=== 5 || item.log_status===1" @click="lxkf" class="item-btn viewbtn">
                    联系客服
                  </view>
...
<script>
  ...
  import { BARGAINING_RECEIVE_NAV_DATAS } from "../../../common/constStatic.js"
  export default {
    data() {
      return {
        current: 0,
    ...
    methods: {
      ...
      // 同意
      agree(item){
        this.$api.post(global.apiUrls.agree_dicker,{
          log_id:item.log_id
        }).then(res => {
          if(res.data.code == 1) {
              const result = res.data.data
      ...
      // 获取数据源
      getDatas(parames){
        this.$api.post(global.apiUrls.my_receive_dicker,parames).then(res => {
          if(res.data.code == 1) {
              const result = res.data.data
