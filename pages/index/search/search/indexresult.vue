<template>
  <view class="page">
	...
	<block v-else>
		<view class="flex flex-direction justify-center align-center bg-white" style="padding: 225rpx 0 225rpx;" :style="[{height:'calc(100vh - '+ CustomBar + 'px)'}]" v-if="!allGameList.length">
		...
		<view v-else class="padding-top" style="background: #f8f8f8;">
			<fu-waterfall v-model="allGameList">
...
export default {
  ...
  onReachBottom() {
	  if(this.pagingListPage < this.last_page) {
		  this.pagingListPage++;
		  this.gamemobile_search();
	  }
  },
  methods: {
	  ...
	  gamemobile_search(status) {
		  ...
		  this.$api.post(global.apiUrls.gamegamegoodslistmobile,{
			  goods_name:this.search_info,
			  ...
			  if(res.data.code == 1) {
				  if(this.pagingListPage == 1) {
					  this.allGameList = res.data.data.data;
				  } else {
					  this.allGameList.push(res.data.data.data)
				  }
