	...
	export default {
		...
		methods: {
			gamegamegoodslist() {
				...
				let that = this;
				uni.request({
				    url:API_BASE_URL + global.apiUrls.huoquyouxiliebiaoshuju, //仅为示例，并非真实接口地址。
				    data: requestParams,
					method:'POST',
				    success: (res) => {
					   ...
					   if (res.data.code == 1) {
					   	...
					   	that.totalSize = Math.ceil(totalSize / 10); // 悬浮按钮总页码
					   	that.currentPage = that.page; //悬浮当前页面
					   	// console.log(res.data.data);
					   	let list = res.data.data.data;
					   	if (that.page == 1) that.lists = [];
					   	that.lists = that.lists.concat(list);
					   	// console.log(this.lists);
					   	if (that.lists.length >= totalSize) {
					   		// this.status = 'noMore';
					   	} else {
					   		// this.status = 'more';
					   		that.page++;
				...
				return;
				this.$api.post(global.apiUrls.huoquyouxiliebiaoshuju, requestParams).then(res => {
					if (res.data.code == 1) {
						let totalSize = res.data.data.total;
