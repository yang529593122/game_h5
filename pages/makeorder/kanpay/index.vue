<template>
	<view class="onlinePayPage">
		...
		<view class="onlinePayPagePrice">
			...
			<view class="flex align-center justify-center onlinePayPagePricepay" v-if="type == 1">
				应付砍价定金
			</view>
			<view class="flex align-center justify-center onlinePayPagePricepay" v-if="type == 2">
				追加砍价定金
			</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			searchOrder() {
				this.$api.post(global.apiUrls.searchOrder,{
					order_no:this.orderNo
				}).then(res => {
					if(res.data.code == 1) {
						console.log('222')
			...
			yuePay() {
				this.$util.throttle(() => {
					this.$api.post(global.apiUrls.gamepayorder,{
						...
						if(res.data.code == 1) {
							// uni.redirectTo({
							// 	url:'/pages/makeorder/kanpay/index?orderNo=' + this.orderNo
							// })
							uni.redirectTo({
			...
			wxPay() {
				this.$util.throttle(() => {
					this.$api.post(global.apiUrls.gamepayorder,{
						...
						client_type:2,
						order_no:this.orderNo,
						return_url:SHAREURL + '/pages/makeorder/kanpay/index?orderNo=' + this.orderNo,
					}).then(res => {
						if(res.data.code == 1) {
							// window.location.href = res.data.data.mweb_url;
							window.location.href = res.data.data.expend.wx_h5_pay_url 
							+ '&redirect_url=' + encodeURIComponent(SHAREURL + 'pages/makeorder/kanpay/index?orderNo=' + this.orderNo);
						} else {
							this.$message.info(res.data.msg);
						}
						this.confirmPayLoading = false;
