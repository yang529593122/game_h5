<template>
	<view class="saleaccountPage">
		...
		<view class="saleaccountPageSwiper">
			<swiper circular :indicator-dots="false" :autoplay="true" :interval="3000" :duration="1000"
				...
				<swiper-item class="saleaccountPageSwiper" v-for="(item,index) in swpierList" :key="index">
					<view class="saleaccountPageSwiper">
						<fu-image :src="item.thumb" mode=""></fu-image>
		...
		<!-- <view style="width: 400rpx;margin-left: 175rpx;margin-top: 20rpx;"> -->
			<fu-tab :tabs="MoreListGame" @change="changeTab" bottom="0" :current="current" node-key="id" current-key="id" :scrollspy="true" size="28" inactive-color="#36373D" active-color="#295B7B"></fu-tab>
		...
		<fu-popup v-model="showRules" mode="center">
			<view class="bg-white" style="width: 580rpx;height: 461rpx;border-radius: 8rpx;">
				...
				<view class="" style="padding: 0 38rpx;height: 249rpx;">
					<view @click="SelectPlatform" class="flex flex-direction justify-center align-center" style="width: 100%;height: 142rpx;background: #659BC7;border-radius: 4px;">
					...
					<view @click="SelectPlatformDetail" class="flex flex-direction justify-center align-center" style="width: 100%;height: 142rpx;background: #659BC7;border-radius: 4px;margin-top: 24rpx;">
						<view class="text-lg text-white" style="font-weight: 500;">
							平台代售
						</view>
...
<script>
	export default {
		...
		methods: {
			...
			gotoSaleAccount(it) {
				this.$urouter.navigateTo('/pages/makeorder/selectarea/index?type=2&gameId=' + it.game_id);
			},
			closePopup() {
			...
			SelectPlatform() {
				this.$urouter.navigateTo('/pages/account/platformrecover');
				// this.$urouter.navigateTo('/pages/account/platformsell');
				this.showRules = false;
				// #ifdef H5
