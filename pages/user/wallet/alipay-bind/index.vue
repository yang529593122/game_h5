<template>
	<view class="page-user">
		...
		<!-- 提交按钮 -->
		<view v-if="!loading" class="bottom_btn" @click="handleSubmit">{{i18n['提交']}}</view>
		<!-- 断网检查 -->
	...
	export default {
		data() {
			return {
				...
				logo: require('../../static/up02.png') , // {String} 收款图片
        		qrcode:'',
		...
		onLoad() {
			this.setNavigationBarTitle(this.i18n['绑定支付宝'])
			this.getMobile();
			this.Detail();
		},
		methods: {
			 ...
			 */
			async handleSubmit() {
				console.log(this.logo,this.qrcode)
				let reg = /^[\u0391-\uFFE5]+$/;
				let _this = this
