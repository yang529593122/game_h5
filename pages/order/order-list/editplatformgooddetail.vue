		...
		<block v-else>
			...
			<view style="width: 702rpx;margin-top: 24rpx;margin-left: 24rpx;" class="bg-white">
				...
				<view style="padding: 32rpx 24rpx 0;" class="flex align-center">
					...
					<view style="width: 502rpx;" class="text-sm">
						<input placeholder="请输入挂号金额" v-model="price" type="digit" style="font-size: 24rpx;color: #333;" placeholder-style="color: #B7BAC2;font-size:24rpx"  />
					</view>
				</view>
				<!-- 商品描述 -->
				<view style="padding: 32rpx 24rpx 0;" class="flex align-center">
					...
					<view style="width: 502rpx;" class="text-sm">
						<input placeholder="请输入号主留言" v-model="detail" style="font-size: 24rpx;color: #333;" placeholder-style="color: #B7BAC2;font-size:24rpx"  />
					</view>
				</view>
				<!-- 商品截图 -->
				...
				<view style="padding: 32rpx 24rpx 0;" class="flex align-center">
					...
					<view style="width: 502rpx;color: #868686;" class="text-sm flex align-center">
						<!-- {{game_accout}} -->
						<view class="flex align-center" @click.stop="changeKanJia(1)">
							<view style="width: 24rpx;height: 24rpx;">
								<image v-if="is_bargain == 1" src="../static/xz.png" style="width: 24rpx;height: 24rpx;"></image>
								<image v-else src="../static/mxz.png" style="width: 24rpx;height: 24rpx;"></image>
							</view>
							<view class="margin-left-xs" :style="[{color:is_bargain == 1 ? '#659BC7' : '#868686'}]">是</view>
						</view>
						
						<view class="flex align-center margin-left-lg"  @click.stop="changeKanJia(0)">
							<view style="width: 24rpx;height: 24rpx;">
								<image v-if="is_bargain == 0" src="../static/xz.png" style="width: 24rpx;height: 24rpx;"></image>
								<image v-else src="../static/mxz.png" style="width: 24rpx;height: 24rpx;"></image>
							</view>
							<view class="margin-left-xs" :style="[{color:is_bargain == 0 ? '#659BC7' : '#868686'}]">否</view>
						</view>
					</view>
				</view>
				<!-- 游戏账号 -->
			...
			<view style="width: 702rpx;" class="">
				...
				<view @click="changeXuanze(index)" style="width: 702rpx;padding: 20rpx;margin-left: 24rpx;border-radius: 4rpx;margin-bottom: 16rpx;"
					class=" flex align-center" v-for="(item,index) in ruleList" :key="index">
...
<button type="default" :loading="isBtnLoading" @click="confirmSubmit()" class="flex align-center justify-center text-lg xiamiananniu">提交</button>
<view style="height: 24rpx;"></view>
		</block>
	</view>
</template>
			...
			confirmSubmit() {
				...
				this.$util.throttle(() => {
					this.$api.post(global.apiUrls.gameusereditgoods,{
						...
						user_images_audit:this.detail_images.map(i => i.thumb).join(',')
					}).then(res => {
			...
			gameusereditgoods() {
				...
				this.$api.post(global.apiUrls.gameusereditgoods, {
					...
					if (res.data.code == 1) {
						...
						this.goodsInfo = {
							game_logo:res.data.data.cover_images[0] || res.data.data.logo,
